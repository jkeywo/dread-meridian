...
for asset in ['gunman','bruiser','lookout','bomber','gang-boss']:
    ...
    task=unreal.AssetImportTask();task.filename=str(p/(asset+'-rigged.fbx'));task.destination_path='/Game/DreadMeridian/Characters/Enemies/Smugglers/'+asset.title().replace('-','')
    task.destination_name='SKM_'+asset.title().replace('-','');task.automated=True;task.replace_existing=True;task.save=True
    ...
    for i,slot in enumerate(slots):
        ...
        name='M_'+asset.title().replace('-','')+'_'+key(slot.material_slot_name)
