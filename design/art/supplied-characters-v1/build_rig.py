...
# Exclude placement tracks belonging to the reference preview scene.
...
for side,s in [('l',1),('r',-1)]:
    ...
    for i,(finger,dy,length) in enumerate([('index',-.027,.074),('middle',-.008,.088),('ring',.012,.081),('pinky',.030,.065)]):
        ...
        if False:y=[-.004,.016,.037,.057][i];start=.88;length=[.084,.095,.089,.071][i]
...
# One torso fit avoids independent spine stretches pulling collars into ridges.
# Bone rotations/hierarchy remain stock; source trunk dimensions stay coherent.
trunk=Matrix.Identity(4)
trunk[2][2]=((ref['upperarm_l'].translation.z+ref['upperarm_r'].translation.z)*.5-ref['pelvis'].translation.z)/(c['shoulder']-c['hip'])
trunk.translation=ref['pelvis'].translation-trunk.to_3x3()@points['pelvis']
for name in ['pelvis']+spines+['neck_01','neck_02','head']:
    source[name]=trunk.inverted()@ref[name]
...
# Resolve exported images to the preserved project texture files.
for mat in mesh.data.materials:
    for node in mat.node_tree.nodes:
        if node.type=='TEX_IMAGE' and node.image:
            key=next((k for k in ['basecolor','normal','roughness','metallic','rm'] if k in node.image.name.lower()),None)
            if key:
                path=OUT/'textures'/f'{asset}-{key}.png'
                if path.exists():
...
arms=(x>c['sx']*.85)&(height>z-(.40 if asset=='medium' else .23))&(height<z+.23)
heads=(height>c['neck']-.025)&(x<c['sx']*.85)&~arms
legs=(height<c['hip']-.065)&~arms
body=~(arms|heads|legs)
linear(heads,['neck_01','neck_02','head'],[c['neck']-.04,c['neck']+.005,c['head']-.07],height[heads])
...
# Diffuse joint transitions along actual mesh edges, avoiding rigid regional seams.
edge=np.array([e.vertices[:] for e in mesh.data.edges],dtype=np.int32)
a=edge[:,0];b=edge[:,1];degree=np.bincount(np.concatenate([a,b]),minlength=len(verts))
for iteration in range(5):
    acc=np.zeros_like(weights)
    np.add.at(acc,a,weights[b]);np.add.at(acc,b,weights[a])
    average=acc/np.maximum(1,degree)[:,None]
    mobile=(height>.17)&(height<c['head']-.025)&(x<c['wrist']+.025)
    weights[mobile]=weights[mobile]*.5+average[mobile]*.5
weights/=np.maximum(weights.sum(axis=1)[:,None],1e-15)
for side in ['l','r']:
...
keep=np.argsort(weights,axis=1)[:,-8:];mask=np.zeros_like(weights,dtype=bool)
...
socketdata=json.loads((OUT/(asset+'-sockets.json')).read_text())
sockets=[]
for name,data in socketdata.items():
    bone=data['bone'];local=Matrix(data['matrix_bone_local_metres'])
    world=ref[bone]@local
    ...
    ob['attachment_id']=name;ob['bone']=bone;sockets.append(ob)
for ob in imported:
...
for side,s in [('l',1),('r',-1)]:
    for n in ['upperarm_'+side,'lowerarm_'+side,'hand_'+side]:
        ...
        m=Matrix.LocRotScale(m.translation,m.to_quaternion(),Vector((.01,.01,.01)))
