...
Accepted enemy deaths award each investigator 5 XP, each cleared camp 250, occupation completion 350, encounter victory 650.
...
All four investigator evolution families are implemented in separate commits; see evolved-abilities.md. No claim of controller parity or host-migration restoration.
...
Integration gate: Unreal Foundation, full Editor, Smoke, NetworkTest and SmugglerSmoke; Python export tests and provenance validation of fresh captures.
The network fixture grants accepted developer-only XP, drives a real client evolution RPC, compares each public build with the authority, and still verifies private Madness delivery and disconnect handoff.
All six opportunities are earned before the gang boss through three cleared camps plus occupation completion. The sandbox remains one encounter, not a generated full run. Objective rewards, authored objective/structure actors, Mythos bosses, full-run progression balance, migration and manual controller parity remain unverified/unimplemented.
